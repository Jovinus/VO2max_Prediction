...
# %%
bins = [10, 30, 40, 50, 60, 80]
df_male['Age'] = pd.cut(df_male['AGE'], bins=bins)

mapper = {"(10, 30]":25, "(30, 40]":35, "(40, 50]":45, "(50, 60]":55, '(60, 80]':70}
...
def plot_line(data, x_var, y_var, indi):
    ...
    sns.regplot(x=x_var, y=y_var, data=subset[subset[indi] == 'High'], ax=ax, scatter=False, label='High', order=3, ci=0)
    sns.regplot(x=x_var, y=y_var, data=subset[subset[indi] == 'Moderate'], ax=ax, scatter=False, label='Moderate', order=3, ci=0)
    sns.regplot(x=x_var, y=y_var, data=subset[subset[indi] == 'Low'], ax=ax, scatter=False, label='Low', order=3, ci=0)
    sns.regplot(x=x_var, y=y_var, data=subset, ax=ax, scatter=False, label='Average', order=3, ci=0)
    plt.legend(fontsize=16)
    plt.xlabel(x_var, fontsize=18)
    plt.ylabel(y_var, fontsize=18)
    plt.show()
